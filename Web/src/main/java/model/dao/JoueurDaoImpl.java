...
public class JoueurDaoImpl implements JoueurDao {
	...
	private static final String SQL_SELECT_PAR_EMAIL = "SELECT id, email, nom, mot_de_passe, date_inscription FROM Joueur WHERE email = ?";
	
	private static final String SQL_INSERT = "INSERT INTO Joueur (email, mot_de_passe, nom, date_inscription) VALUES (?, ?, ?, NOW())";
	...
	@Override
	public Joueur trouver( String email ) throws DAOException {
	...
	public void creer( Joueur Joueur ) throws DAOException {
		...
		try {
			/* Récupération d'une connexion depuis la Factory */
			connexion = daoFactory.getConnection();
			preparedStatement = DAOUtilitaire.initialisationRequetePreparee( connexion, SQL_INSERT, true, Joueur.getUsername(), Joueur.getMotDePasse(), Joueur.getNbPieces() );
			int statut = preparedStatement.executeUpdate();
			/* Analyse du statut retourné par la requête d'insertion */
			if ( statut == 0 ) {
				throw new DAOException( "Échec de la création de l'Joueur, aucune ligne ajoutée dans la table." );
			}
			...
			if ( valeursAutoGenerees.next() ) {
				...
				throw new DAOException( "Échec de la création de l'Joueur en base, aucun ID auto-généré retourné." );
			}
		} catch ( SQLException e ) {
			throw new DAOException( e );
		} finally {
